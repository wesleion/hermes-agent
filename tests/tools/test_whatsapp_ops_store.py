...
def test_init_db_adds_resolved_contact_id_to_legacy_inbound_events(tmp_path):
    from tools.whatsapp_ops_store import init_db
    ...
    try:
        ...
        init_db()
        with sqlite3.connect(db_path) as conn:
            columns = {row[1] for row in conn.execute("PRAGMA table_info(inbound_events)")}
    finally:
        reset_hermes_home_override(token)

    assert "resolved_contact_id" in columns
...
def test_resolve_inbound_contact_by_lid_maps_quepasa_phone_to_registered_channel(
    ...
    class Response:
        ...
        def read(self, size):
            ...
            return b'{"success": true, "phone": "551188887777"}'
    ...
    try:
        ...
        resolved = resolve_inbound_contact_by_lid("123456789@lid")
    finally:
        reset_hermes_home_override(token)

    assert resolved == registered["contact_id"]
    assert captured["request"].get_method() == "GET"
    assert captured["request"].full_url == (
        "https://quepasa.example.invalid/useridentifier?lid=123456789%40lid"
    )
    assert captured["request"].get_header("X-quepasa-token") == "secret-token"
    assert captured["timeout"] == 15
    assert captured["read_size"] == 4096


def test_resolve_inbound_contact_by_lid_fails_closed_without_network_for_non_lid(
    tmp_path, monkeypatch
):
    from tools.whatsapp_ops_store import init_db, resolve_inbound_contact_by_lid
    ...
    try:
        init_db()
        assert resolve_inbound_contact_by_lid("551188887777@s.whatsapp.net") == ""
        assert calls == []
        assert resolve_inbound_contact_by_lid("123456789@lid") == ""
    finally:
        reset_hermes_home_override(token)
...
def test_staging_peek_and_diagnostics_do_not_delete_expired_rows(tmp_path):
    from tools.whatsapp_ops_store import (
        get_db_path,
        init_db,
        peek_staging,
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            conn.execute(
                ...
                """,
                (
                    "staging_expired_readonly",
                    "contact-redacted-for-test",
                    "thread-redacted-for-test",
                    "Contato Expirado",
                    "contact",
                    expired_at,
                    expired_at,
                    "hash-expired-readonly",
                    expired_at,
                    1,
                    json.dumps({"display_name": "Contato Expirado"}),
                ),
            )
        assert peek_staging() == []
        diagnostics = registration_staging_diagnostics()
        with sqlite3.connect(get_db_path(tmp_path)) as conn:
            remaining = conn.execute(
                "SELECT COUNT(*) FROM registration_staging WHERE id=?",
                ("staging_expired_readonly",),
            ).fetchone()[0]
    finally:
        reset_hermes_home_override(token)
