...
| `/missao [what to accomplish]` | Transform a rough request into a Mission Contract with clarify, gates, execution mode, success verifier, and evidence reporting. Works in the CLI, TUI, and messaging gateway. |
...
| `/missao [what to accomplish]` | Start a Mission Intake Loop: classify the request, ask only necessary `clarify` questions, draft a Mission Contract, route to the right execution mode, and validate against success criteria. |
...
- `/status`, `/version`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, `/credits`, `/suggestions`, `/blueprint`, `/learn`, `/missao`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
