...
def _normalize_useridentifier_url(raw_url: str) -> str:
    """Resolve `/useridentifier` from either a QuePasa base or send endpoint."""
    raw_url = str(raw_url or "").strip()
    parsed = urllib.parse.urlsplit(raw_url)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        return ""
    ...
    for suffix in (
        ...
        if path.endswith(suffix):
            path = path[: -len(suffix)]
            break
    normalized_path = f"{path}/useridentifier" if path else "/useridentifier"
    return urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, normalized_path, "", ""))
...
def _resolve_lid_participant(raw_url: str, api_key: str, participant: str) -> str:
    ...
    if not value.endswith("@lid"):
        return value
    useridentifier_url = _normalize_useridentifier_url(raw_url)
...
def resolve_lid_via_quepasa(lid_ref: str) -> str:
    """Resolve one inbound LID through QuePasa's read-only endpoint.

    Credentials and endpoint normalization stay in this transport module.
    Empty string means fail closed; this helper never sends or mutates provider
    state.
    """
    try:
        ...
        if not raw_url or not api_key:
            return ""
        return _resolve_lid_participant(raw_url, api_key, value)
    except Exception:
        return ""
