...
from tools.whatsapp_ops_store import (
    ...
    peek_staging,
    pause_campaign,
    persist_inbound_event_resolution,
    ...
    resolve_inbound_contact_by_lid,
    _payload_from_self,
    resolve_contact,
    stage_raw_ref,
...
def _notify_operator_of_recognized_inbound(
    contact_id: str, cfg: dict[str, Any]
) -> dict[str, Any]:
    """Notify the operator that a known allowlisted contact replied."""
    try:
        contact_value = str(contact_id or "").strip()
        if not contact_value:
            return {"ok": False, "reason": "contact_id_missing"}

        from tools.whatsapp_ops_store import _connect as _wpp_connect

        with _wpp_connect() as conn:
            row = conn.execute(
                "SELECT display_name, whitelisted FROM contacts WHERE id=?",
                (contact_value,),
            ).fetchone()
        if row is None or not row["whitelisted"]:
            return {"ok": False, "reason": "contact_not_allowlisted"}
        ...
        text = (
            f"💬 <b>{safe_name}</b> respondeu no WhatsApp.\n\n"
            "O Hunter pode preparar um draft de continuidade se você quiser -- "
            "é só pedir aqui mesmo "
            f'(ex: "prepara uma resposta pro {safe_name}").'
        )
        ...
        return {
            ...
            "message_id": (
                str((result or {}).get("message_id", ""))
                if isinstance(result, dict)
                else ""
            ),
        }
    except Exception as exc:
        return {"ok": False, "reason": "notification_error", "error": str(exc)[:120]}


def _approval_card_target_summary(draft: dict[str, Any] | None) -> str:
    """Resolve a human-friendly label for the card header: contact/group name(s).

    Falls back to a generic count when identity cannot be resolved, but never
    exposes raw phone numbers or WhatsApp refs — only local display_name/alias
    already stored in the sanitized contacts/lists cache.
    """
    try:
    ...
    if not isinstance(targets, list) or not targets:
        return "destinatário não identificado"
    labels: list[str] = []
    from tools.whatsapp_ops_store import _connect as _wpp_connect  # local import: shared SQLite connection helper
    ...
    try:
        with _wpp_connect() as conn:
            for target in targets:
                ...
                elif ttype in {"group", "list"}:
                    group_id = str(target.get("group_id") or target.get("list_id") or "")
                    row = conn.execute(
                        "SELECT name FROM lists WHERE id=?", (group_id,)
                    ).fetchone()
                    if row and row["name"]:
                        labels.append(f"grupo {row['name']}")
                elif ttype == "group_create":
                    name = str(target.get("name") or target.get("title") or "").strip()
                    if name:
                        labels.append(f"novo grupo {name}")
    except Exception:
        # Schema/connection issues must never break card rendering — fall
        # back to the generic count below instead of raising.
        pass
    if not labels:
        return f"{len(targets)} destinatário(s)"
    if len(labels) == 1:
        return labels[0]
    if len(labels) <= 3:
        return ", ".join(labels)
    return f"{', '.join(labels[:2])} e mais {len(labels) - 2}"
...
def _send_telegram_approval_card(approval: dict[str, Any], draft: dict[str, Any] | None, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    text = (
        "📲 <b>Aprovação de envio WhatsApp</b>\n\n"
        f"Para: <b>{html.escape(_approval_card_target_summary(draft))}</b>\n"
        f"{media_line}\n"
        f"<pre>{text_preview}</pre>\n\n"
        f"{crm_line}\n"
        f"{approval_note}\n\n"
        f"<i>draft={html.escape(str(draft_id))} · approval={html.escape(str(approval_id))}</i>"
    )
    payload: dict[str, Any] = {
        "chat_id": chat_id,
        "text": text,
...
def _score_lead(lead: dict[str, Any], min_confidence: float, rank: int = 0, include_evidence: bool = False) -> dict[str, Any]:
    lead_id = _safe_lead_id(lead.get("lead_id") or lead.get("candidate_id") or lead.get("id") or lead.get("name") or lead.get("empresa"))
    ...
    if awaiting_reply:
        ...
        signals.append(f"{stale_days}d sem avanço")
    if open_tasks:
        breakdown["open_tasks"] = min(0.16, 0.06 + 0.03 * open_tasks)
    ...
    if target:
        breakdown["target"] = 0.1
    if name and (status or context):
        breakdown["data_quality"] = 0.04
    confidence = round(min(sum(breakdown.values()), 0.95), 2)
    ...
    opportunity_id = "opp_" + hash_text(f"{lead_id}|{name}|{status}|{reason}")[:12]
    result = {
        "rank": rank,
        ...
        "manual_reasons": missing_context or (["confidence_below_threshold"] if confidence < min_confidence else []),
        "missing_context": missing_context,
        "recommended_action": recommended_action,
        "recommended_next_action": recommended_action,
        "risk_flags": [] if not manual_required else ["manual_review_required"],
...
def wpp_opportunity_scores(
    ...
    lead_inputs: list[dict[str, Any]] | None = None,
    limit: int = 10,
    ...
    response = {
        ...
        "segment_ref_set": bool(str(segment_ref or "").strip()),
        "thread_set": bool(str(thread or "").strip()),
        "contact_set": bool(str(contact or "").strip()),
        ...
        "count": len(capped),
        "counts": {
            "input_leads": len(input_leads),
            "local_candidates": 0,
            "scored": len(scored),
            "manual_required": sum(1 for item in scored if item.get("manual_required")),
        },
...
def _draft_message_for_opportunity(item: dict[str, Any], objective: str = "") -> str:
    explicit = _safe_commercial_text(item.get("message") or item.get("draft_message"), 500)
    if explicit:
        return explicit
    lead_name = _safe_commercial_text(item.get("lead_name") or item.get("name") or item.get("lead_id") or item.get("candidate_id"), 80) or "esse projeto"
...
def _candidate_id(value: dict[str, Any]) -> str:
    return _safe_lead_id(value.get("candidate_id") or value.get("lead_id") or value.get("id") or value.get("lead_name"))
...
def wpp_proactive_draft_queue(
    ...
    Default mode is preview/read-only. ``mode='create'`` is required for local
    draft/approval writes. This never sends WhatsApp, never writes CRM, never
    ...
    for raw in source_items[:cap]:
        ...
        elif target.get("type") != "contact":
            missing.append("contact_only_v1")
        if action in {"manual_required", "ask_operator", "wait"}:
            missing.append("operator_review")
        if raw_risks:
            missing.append("unsafe_content")
        if dedupe_key in seen_keys:
            missing.append("duplicate_candidate")
        ...
        if not create_mode:
            items.append({
                **base_item,
                ...
                "approval_card": {
                    "mode": "preview_only",
                    "operator_action": "call_again_with_mode_create_to_queue",
...
def wpp_ingest_inbound_event(payload: dict[str, Any]) -> str:
    ...
    try:
        ...
        if (
            ...
            and registration_msg_type != "system"
            and not is_synthetic_contact_sync
        ):
            try:
...
def wpp_register_alias(
    ...
    if not ref_raw:
        ...
        if not consumed:
            hint = "Item de staging não encontrado/expirado. Use /fila e escolha um item válido." if staging_id else "Informe 'ref' como o número/ID do WhatsApp (ex.: /addct Nome | 5511999999999), ou use /addct Nome --item N logo após um inbound novo (fila expira em algumas horas)."
            return _json({
                "ok": False,
                "error": "ref_required",
                "hint": hint,
...
registry.register(
    ...
    schema=_schema(
        ...
        "Rank Hunter commercial opportunities using provided CRM/local context. Deterministic and read-only: never sends WhatsApp, never writes CRM, never creates drafts/approvals, never calls an LLM, and never fetches provider history.",
        {
            ...
            "lead_inputs": {"type": "array", "items": {"type": "object"}},
...
registry.register(
    ...
    schema=_schema(
        "wpp_proactive_draft_queue",
        "Preview or create bounded local proactive commercial drafts plus local approval rows from scored opportunities. Default mode is preview/read-only; mode=create is required for local writes. Never sends WhatsApp, never writes CRM, never calls Telegram by itself, never fetches provider history, and low-confidence/ambiguous/unsafe targets become manual_required.",
        {
